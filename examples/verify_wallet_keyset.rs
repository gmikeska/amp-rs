...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match elements_rpc.get_confidential_address(WALLET_NAME, KNOWN_TREASURY_ADDRESS).await {
        Ok(confidential) => {
            println!("\n📊 Address Check:");
        ...
        Err(e) => {
            ...
            if error_msg.contains("Invalid address") || error_msg.contains("does not refer to a key") {
                println!("\n❌ NO MATCH");
